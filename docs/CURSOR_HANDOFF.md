...
4. **Pictures**: the week as bars (green free, grey work, no numbers), a day as a strip plus a list of blocks. A picture is always its own bubble, sent before the words. Saved blocks on the day strip can be dragged to a new time (15-minute snaps); the drop is sent as `move Work to 8 pm today` and Buffer asks before moving it (`ScheduleImage` in `schedule-image.tsx`, the `dragged` fast path in `bot.ts`).
